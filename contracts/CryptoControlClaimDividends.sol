...
contract CryptoControlClaimDividends is Ownable {
    ...
    // A hashmap to track the dividends that have been given out
    mapping (string => uint256) public dividends;
    uint256 public lastDividendIndex = 0;

    event DividendsClaimed(address indexed dest, uint amount, uint nonce, string userid);

    /**
     * @dev Set the CryptoControlToken contract
     * @param _ccioToken The address of the CryptoControlToken contract
        ...
        bytes32 sighash, uint8 v, bytes32 r, bytes32 s) external payable
    {
        ...
        emit DividendsClaimed(msg.sender, amount, nonce, userid);
    }
}